...
# each pattern has a name label and severity so we can report meaningfully
# patterns ordered by decreasing severity
DANGER_PATTERNS = [
...
def _normalize(text: str) -> str:
    ...
    catches leet-speak and zero-width char tricks
    """
...
def filter_response(response: str, block_mode: bool = False) -> dict:
    ...
    observe mode (default): flag but don't modify text
...
def is_clean(response: str) -> bool:
    """quick boolean — no dict overhead, useful in conditionals"""
    return not filter_response(response, block_mode=False)["filtered"]


def summarize_filter_stats(results: list) -> dict:
    """
    stats on what the defense layer caught across a full batch
    call after filter_batch
    """
